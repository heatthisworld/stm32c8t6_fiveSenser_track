...
int main(void)
{
	uint8_t flag = 1; // 定义用于判断是否开始追踪的标志位
	int16_t speed = 300;
	/*模块初始化*/
	OLED_Init(); // OLED初始化
	Servo_Init();
	...
	while (1)
	{
		...
		// 	// OLED_ShowString(3, 1, "Kd: ");
		// 	// OLED_ShowNum(3, 5, Kd * 100, 4); // 显示放大 100 倍的值
		// 	// OLED_ShowString(5, 1, "StSp:");
		// 	// OLED_ShowNum(4, 6, steerStep, 3);
		// }
		// else if (key == 12)
		// {
		// 	// 按键3、4都按下
		// 	steerStep += 0.1f; // 增加舵机调整步长
		// 	OLED_ShowNum(4, 6, steerStep * 100, 3);
		// }
		// else if (key == 13)
		// {
		// 	// 按键1、3,4都按下
		// 	steerStep -= 0.1f; // 减少舵机调整步长
		// 	OLED_ShowNum(4, 6, steerStep * 100, 3);
		// }
		// else if (key == 14)
		// {
		// 	// 按键2、3,4都按下,切换显示,显示steerStep
		// 	// 假设你已经实现 OLED 显示功能
		// 	OLED_Clear();
		// 	OLED_ShowString(1, 1, "Kp: ");
		// 	OLED_ShowNum(1, 5, Kp, 4);
		...
		// 	OLED_ShowNum(3, 5, key, 4);
		// 	OLED_ShowString(4, 1, "StSp:");
		// 	OLED_ShowNum(4, 6, steerStep * 100, 3);
		// }
		// else if (key == 15)
		// {
